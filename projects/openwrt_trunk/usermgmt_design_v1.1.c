...
一： 方案设计：
    ...
    （2） 限速
    （3） portal
    （4） 用户其他配置（如无线黑名单， user_config_host_name）
...
三： 限速
    ...
    限速四种场景：
    （1） 根据mac的限速， 区分intf, 如（ra0, ra1）。
    （2） 根据mac的限速， 不区分intf。
    （3） 根据intf的限速， 参照252cpa， 根据ssid的限速。
...
配置项：1. 场景配置     ----  g_bwlimit_type
    ...
    3.1 用户空间数据
        ...
        typedef struct
        {
            ...
            unsigned char       devname[IFNAMSIZ];                           /* e.g. eth0, ath1, ath2 */
            unsigned char       bwlimit_type;                                /* g_bwlimit_type_int 为1 时， 用以区分根据mac限速时， 要不要区分intf */
...
6.2 限速
    ...
    6.2.1 设置 bwlimit 类型（0, 不限速； 1， 根据mac的; 2, 还是根据intf的）
